...
const records = lines.slice(1).map(line => {
    // Simple regex for CSV splitting that respects quotes
    // Matches: quoted field OR non-quoted field
    const regex = /(".*?"|[^",\s]+)(?=\s*,|\s*$)/g;
    // Actually the robust regex for splitting CSV lines including empty fields:
    // (?:^|,)(?=[^"]|")("?((?<=")[^"]*(?=")|[^",]*)"?)/g
    // But let's use a simpler split with a lookahead for commas not inside quotes
...
newRecords.forEach(newRec => {
    if (!records.find(r => r.repo_id === newRec.repo_id)) {
        ...
        // Update check? No, just keep existing unless force update logic needed.
        // But verified fields might need updating. For now, assume good.
    }
});
...
const finalRecords = records.map(record => {
    ...
    if (repo_id.startsWith('BR-AGG') ||
        repo_id === 'BR-RES-0002' ||
        name.includes('IBICT') ||
        (name.includes('SciELO') || sigla === 'SciELO')) {
    ...
    if (LAYER_3_TARGETS.includes(sigla) ||
        ...
        (repo_name && repo_name.includes('Infoteca')) ||
...
console.log(`Updated ${finalRecords.length} records manually (CJS).`);
