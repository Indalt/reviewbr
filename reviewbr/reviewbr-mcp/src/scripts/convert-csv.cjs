    ...
    const jsonOutput = records.map(record => {
        return {
            ...
            institution: {
                ...
                type: record.ies_tipo === 'Universidade' ? 'federal' : (record.ies_tipo === 'Instituto Federal' ? 'federal' : 'other'),
                state: record.ies_uf,
            ...
            layer: record.camada_metodo
        };
    });

    fs.writeFileSync(jsonPath, JSON.stringify(jsonOutput, null, 2));
    console.log(`Successfully converted ${jsonOutput.length} records to JSON (CJS).`);
